...
readonly DXAI_TM_THIN_TIMEOUT=180
readonly DXAI_TM_THIN_VALUE=9999999999
readonly DXAI_SQLITE_MAX_SIZE=104857600 # 100MB
...
opt_saved_state_cleanup() {
    if [[ "${MO_DEBUG:-}" == "1" ]]; then
        ...
        debug_operation_detail "Method" "Find and remove .savedState folders older than $DXAI_SAVED_STATE_AGE_DAYS days"
    ...
    if [[ -d "$state_dir" ]]; then
        ...
        done < <(command find "$state_dir" -type d -name "*.savedState" -mtime "+$DXAI_SAVED_STATE_AGE_DAYS" -print0 2> /dev/null)
    fi
...
opt_fix_broken_configs() {
    ...
    if [[ -t 1 ]]; then
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "Checking preferences..."
...
opt_sqlite_vacuum() {
    ...
    local spinner_started="false"
    if [[ "${DXAI_DRY_RUN:-0}" != "1" && -t 1 ]]; then
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "Optimizing databases..."
        spinner_started="true"
    fi
    ...
    for pattern in "${db_paths[@]}"; do
        while IFS= read -r db_file; do
            ...
            if [[ "$file_size" -gt "$DXAI_SQLITE_MAX_SIZE" ]]; then
                skipped=$((skipped + 1))
                continue
            fi
...
opt_launch_services_rebuild() {
    ...
    if [[ -t 1 ]]; then
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "Repairing LaunchServices..."
    fi
...
opt_bluetooth_reset() {
    ...
    local spinner_started="false"
    if [[ -t 1 ]]; then
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "Checking Bluetooth..."
        spinner_started="true"
    fi
