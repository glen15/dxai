#!/bin/bash
# dxai - Base Definitions and Utilities
# Core definitions, constants, and basic utility functions used by all modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${DXAI_BASE_LOADED:-}" ]]; then
    return 0
fi
readonly DXAI_BASE_LOADED=1
...
# ============================================================================
readonly DXAI_TEMP_FILE_AGE_DAYS=7       # Temp file retention (days)
readonly DXAI_ORPHAN_AGE_DAYS=60         # Orphaned data retention (days)
readonly DXAI_MAX_PARALLEL_JOBS=15       # Parallel job limit
readonly DXAI_MAIL_DOWNLOADS_MIN_KB=5120 # Mail attachment size threshold
readonly DXAI_MAIL_AGE_DAYS=30           # Mail attachment retention (days)
readonly DXAI_LOG_AGE_DAYS=7             # Log retention (days)
readonly DXAI_CRASH_REPORT_AGE_DAYS=7    # Crash report retention (days)
readonly DXAI_SAVED_STATE_AGE_DAYS=30    # Saved state retention (days) - increased for safety
readonly DXAI_TM_BACKUP_SAFE_HOURS=48    # TM backup safety window (hours)
readonly DXAI_MAX_DS_STORE_FILES=500     # Max .DS_Store files to clean per scan
readonly DXAI_MAX_ORPHAN_ITERATIONS=100  # Max iterations for orphaned app data scan
...
detect_architecture() {
    if [[ -n "${DXAI_ARCH_CACHE:-}" ]]; then
        echo "$DXAI_ARCH_CACHE"
        return 0
    fi

    if [[ "$(uname -m)" == "arm64" ]]; then
        export DXAI_ARCH_CACHE="Apple Silicon"
    else
        export DXAI_ARCH_CACHE="Intel"
    fi
    echo "$DXAI_ARCH_CACHE"
}
...
get_darwin_major() {
    if [[ -n "${DXAI_DARWIN_MAJOR_CACHE:-}" ]]; then
        echo "$DXAI_DARWIN_MAJOR_CACHE"
        return 0
    fi
    ...
    export DXAI_DARWIN_MAJOR_CACHE="$major"
    echo "$major"
}
...
get_optimal_parallel_jobs() {
    ...
    if [[ -z "${DXAI_CPU_CORES_CACHE:-}" ]]; then
        export DXAI_CPU_CORES_CACHE=$(sysctl -n hw.ncpu 2> /dev/null || echo 4)
    fi
    local cpu_cores="$DXAI_CPU_CORES_CACHE"
...
get_invoking_user() {
    if [[ -n "${_DXAI_INVOKING_USER_CACHE:-}" ]]; then
        echo "$_DXAI_INVOKING_USER_CACHE"
        return 0
    fi
    ...
    export _DXAI_INVOKING_USER_CACHE="$user"
    echo "$user"
}
...
get_brand_name() {
    ...
    if [[ -z "${DXAI_IS_CHINESE_SYSTEM:-}" ]]; then
        local sys_lang
        sys_lang=$(defaults read -g AppleLanguages 2> /dev/null | grep -o 'zh-Hans\|zh-Hant\|zh' | head -1 || echo "")
        if [[ -n "$sys_lang" ]]; then
            export DXAI_IS_CHINESE_SYSTEM="true"
        else
            export DXAI_IS_CHINESE_SYSTEM="false"
        fi
    fi

    local is_chinese="${DXAI_IS_CHINESE_SYSTEM}"
...
declare -a DXAI_TEMP_FILES=()
declare -a DXAI_TEMP_DIRS=()
...
cleanup_temp_files() {
    ...
    local file
    if [[ ${#DXAI_TEMP_FILES[@]} -gt 0 ]]; then
        for file in "${DXAI_TEMP_FILES[@]}"; do
            [[ -f "$file" ]] && rm -f "$file" 2> /dev/null || true
        done
    fi

    if [[ ${#DXAI_TEMP_DIRS[@]} -gt 0 ]]; then
        for file in "${DXAI_TEMP_DIRS[@]}"; do
...
start_section_spinner() {
    ...
    stop_inline_spinner || true
    if [[ -t 1 ]]; then
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "$message"
    fi
}
...
is_ansi_supported() {
    if [[ -n "${DXAI_ANSI_SUPPORTED_CACHE:-}" ]]; then
        return "$DXAI_ANSI_SUPPORTED_CACHE"
    fi

    # Check if running in interactive terminal
    if ! [[ -t 1 ]]; then
        export DXAI_ANSI_SUPPORTED_CACHE=1
        return 1
    fi
    ...
    case "$TERM" in
        xterm* | vt100 | vt220 | screen* | tmux* | ansi | linux | rxvt* | konsole*)
            export DXAI_ANSI_SUPPORTED_CACHE=0
            return 0
            ;;
        dumb | unknown)
            export DXAI_ANSI_SUPPORTED_CACHE=1
