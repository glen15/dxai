#!/bin/bash
# dxai - Common Functions Library
# Main entry point that loads all core modules

set -euo pipefail

# Prevent multiple sourcing
if [[ -n "${DXAI_COMMON_LOADED:-}" ]]; then
    return 0
fi
readonly DXAI_COMMON_LOADED=1
...
source "$_DXAI_CORE_DIR/file_ops.sh"
source "$_DXAI_CORE_DIR/help.sh"
source "$_DXAI_CORE_DIR/ui.sh"
source "$_DXAI_CORE_DIR/app_protection.sh"

# Load sudo management if available
if [[ -f "$_DXAI_CORE_DIR/sudo.sh" ]]; then
    source "$_DXAI_CORE_DIR/sudo.sh"
fi
...
update_via_homebrew() {
    ...
    # Upgrade dxai
    if [[ -t 1 ]]; then
        start_inline_spinner "Upgrading dxai..."
    else
        echo "Upgrading dxai..."
    fi

    brew upgrade dxai > "$temp_upgrade" 2>&1 &
    local upgrade_pid=$!
    ...
    if echo "$upgrade_output" | grep -q "already installed"; then
        ...
        installed_version=$(brew list --versions dxai 2> /dev/null | awk '{print $2}')
        [[ -z "$installed_version" ]] && installed_version=$(mo --version 2> /dev/null | awk '/dxai version/ {print $3; exit}')
        echo ""
        ...
        local new_version
        new_version=$(brew list --versions dxai 2> /dev/null | awk '{print $2}')
        [[ -z "$new_version" ]] && new_version=$(mo --version 2> /dev/null | awk '/dxai version/ {print $3; exit}')
        echo ""
    ...
    rm -f "$HOME/.cache/dxai/version_check" "$HOME/.cache/dxai/update_message" 2> /dev/null || true
}
