...
if [[ -n "${DXAI_UI_LOADED:-}" ]]; then
    return 0
fi
readonly DXAI_UI_LOADED=1
...
read_key() {
    ...
    if [[ "${DXAI_READ_KEY_FORCE_CHAR:-}" == "1" ]]; then
        [[ -z "$key" ]] && {
            echo "ENTER"
            return 0
        }
...
start_inline_spinner() {
    ...
    if [[ -t 1 ]]; then
        ...
        INLINE_SPINNER_STOP_FILE="${TMPDIR:-/tmp}/dxai_spinner_$$_$RANDOM.stop"
        ...
        (
            ...
            while [[ ! -f "$stop_file" ]]; do
                ...
                printf "\r${DXAI_SPINNER_PREFIX:-}${BLUE}%s${NC} %s" "$c" "$display_message" >&2 || break
                i=$((i + 1))
                sleep 0.05
            done
...
has_full_disk_access() {
    ...
    if [[ -n "${DXAI_HAS_FDA:-}" ]]; then
        if [[ "$DXAI_HAS_FDA" == "1" ]]; then
            return 0
        elif [[ "$DXAI_HAS_FDA" == "unknown" ]]; then
            return 2
        else
            return 1
        fi
