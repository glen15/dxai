#!/bin/bash
# dxai - Application Protection
# System critical and data-protected application lists

set -euo pipefail

if [[ -n "${DXAI_APP_PROTECTION_LOADED:-}" ]]; then
    return 0
fi
readonly DXAI_APP_PROTECTION_LOADED=1

_DXAI_CORE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
[[ -z "${DXAI_BASE_LOADED:-}" ]] && source "$_DXAI_CORE_DIR/base.sh"
...
#
# In uninstall mode (DXAI_UNINSTALL_MODE=1), only system-critical components are protected.
...
should_protect_path() {
    ...
    if [[ "$path" =~ /Library/Containers/([^/]+) ]] || [[ "$path" =~ /Library/Group\ Containers/([^/]+) ]]; then
        ...
        if [[ "${DXAI_UNINSTALL_MODE:-0}" != "1" ]] && should_protect_data "$bundle_id"; then
            return 0
        fi
    fi
    ...
    # This catches things like /Users/username/Library/Caches/Claude when pattern is *Claude*
    # In uninstall mode, only check system-critical bundles (user explicitly chose to uninstall)
    if [[ "${DXAI_UNINSTALL_MODE:-0}" == "1" ]]; then
