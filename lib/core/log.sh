...
if [[ -n "${DXAI_LOG_LOADED:-}" ]]; then
    return 0
fi
readonly DXAI_LOG_LOADED=1
...
readonly LOG_FILE="${HOME}/.config/dxai/dxai.log"
readonly DEBUG_LOG_FILE="${HOME}/.config/dxai/dxai_debug_session.log"
readonly OPERATIONS_LOG_FILE="${HOME}/.config/dxai/operations.log"
...
rotate_log_once() {
    ...
    [[ -n "${DXAI_LOG_ROTATED:-}" ]] && return 0
    export DXAI_LOG_ROTATED=1
...
log_system_info() {
    ...
    [[ -n "${DXAI_SYS_INFO_LOGGED:-}" ]] && return 0
    export DXAI_SYS_INFO_LOGGED=1
    ...
    {
        echo "----------------------------------------------------------------------"
        echo "dxai Debug Session, $(date '+%Y-%m-%d %H:%M:%S')"
        echo "----------------------------------------------------------------------"
        echo "User: $USER"
