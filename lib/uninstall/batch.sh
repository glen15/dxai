...
[[ -z "${DXAI_COMMON_LOADED:-}" ]] && source "$SCRIPT_DIR/lib/core/common.sh"
...
batch_uninstall_applications() {
    ...
    trap 'stop_inline_spinner 2>/dev/null; _cleanup_sudo_keepalive; unset DXAI_UNINSTALL_MODE; echo ""; _restore_uninstall_traps; return 130' INT TERM
    ...
    if [[ ${#sudo_apps[@]} -gt 0 && "${DXAI_DRY_RUN:-0}" != "1" ]]; then
    ...
    if [[ $brew_apps_removed -gt 0 && "${DXAI_DRY_RUN:-0}" != "1" ]]; then
        (
