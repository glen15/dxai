...
ask_for_updates() {
    ...
    if [[ -n "${DXAI_UPDATE_AVAILABLE:-}" && "${DXAI_UPDATE_AVAILABLE}" == "true" ]]; then
        has_updates=true
    fi
    ...
    if [[ "${DXAI_UPDATE_AVAILABLE:-}" == "true" ]]; then
        echo -ne "${YELLOW}Update dxai now?${NC} ${GRAY}Enter confirm / ESC cancel${NC}: "
...
perform_updates() {
    # Only handle dxai updates here; Homebrew/App Store/macOS are manual (tips shown in ask_for_updates)
    local updated_count=0
    local total_count=0

    if [[ -n "${DXAI_UPDATE_AVAILABLE:-}" && "${DXAI_UPDATE_AVAILABLE}" == "true" ]]; then
        echo -e "${BLUE}Updating dxai...${NC}"
        local dxai_bin="${SCRIPT_DIR}/../../dxai"
        [[ ! -f "$dxai_bin" ]] && dxai_bin=$(command -v dxai 2> /dev/null || echo "")

        if [[ -x "$dxai_bin" ]]; then
            if "$dxai_bin" update 2>&1 | grep -qE "(Updated|latest version)"; then
                echo -e "${GREEN}${ICON_SUCCESS}${NC} dxai updated"
                reset_dxai_cache
                updated_count=$((updated_count + 1))
            else
                echo -e "${RED}✗${NC} dxai update failed"
            fi
        else
            echo -e "${RED}✗${NC} dxai executable not found"
        fi
        echo ""
        total_count=1
    fi
