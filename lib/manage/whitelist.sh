...
_DXAI_MANAGE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$_DXAI_MANAGE_DIR/../core/common.sh"
source "$_DXAI_MANAGE_DIR/../ui/menu_simple.sh"

# Config file paths
readonly WHITELIST_CONFIG_CLEAN="$HOME/.config/dxai/whitelist"
readonly WHITELIST_CONFIG_OPTIMIZE="$HOME/.config/dxai/whitelist_optimize"
readonly WHITELIST_CONFIG_OPTIMIZE_LEGACY="$HOME/.config/dxai/whitelist_checks"
...
dxai updates check|check_dxai_update|update_check
...
${GRAY}Edit: ${display_config}${NC}"
    ...
    if [[ ${#selected_cache_patterns[@]} -gt 0 ]]; then
        ...
        local IFS=','
        export DXAI_PRESELECTED_INDICES="${preselected_indices[*]}"
    else
        unset DXAI_PRESELECTED_INDICES
    fi

    DXAI_SELECTION_RESULT=""
    ...
    if [[ -n "$DXAI_SELECTION_RESULT" ]]; then
        local -a selected_indices
        IFS=',' read -ra selected_indices <<< "$DXAI_SELECTION_RESULT"
