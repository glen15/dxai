...
reset_dxai_cache() {
    clear_cache_file "$CACHE_DIR/dxai_version"
}
...
check_homebrew_updates() {
    ...
    if [[ "$use_cache" == "false" ]]; then
        ...
        if [[ -t 1 ]]; then
            DXAI_SPINNER_PREFIX="  " start_inline_spinner "Checking Homebrew updates..."
...
check_macos_update() {
    ...
    if [[ $(get_software_updates) == "Updates Available" ]]; then
        ...
        local spinner_started=false
        if [[ -t 1 ]]; then
            DXAI_SPINNER_PREFIX="  " start_inline_spinner "Checking macOS updates..."
            spinner_started=true
        fi
...
check_dxai_update() {
    if command -v is_whitelisted > /dev/null && is_whitelisted "check_dxai_update"; then return; fi

    # Check if dxai has updates
    # Auto-detect version from dxai main script
    local current_version
    if [[ -f "${SCRIPT_DIR:-/usr/local/bin}/dxai" ]]; then
        current_version=$(grep '^VERSION=' "${SCRIPT_DIR:-/usr/local/bin}/dxai" 2> /dev/null | head -1 | sed 's/VERSION="\(.*\)"/\1/' || echo "unknown")
    else
        current_version="${VERSION:-unknown}"
    fi

    local latest_version=""
    local cache_file="$CACHE_DIR/dxai_version"

    export DXAI_UPDATE_AVAILABLE="false"
    ...
    if is_cache_valid "$cache_file"; then
        ...
        if [[ -t 1 ]]; then
            DXAI_SPINNER_PREFIX="  " start_inline_spinner "Checking dxai version..."
        fi
        ...
        if command -v curl > /dev/null 2>&1; then
            ...
            local temp_version
            temp_version=$(mktemp_file "dxai_version_check")
            curl -fsSL --connect-timeout 3 --max-time 5 https://api.github.com/repos/glen15/dxai/releases/latest 2> /dev/null | grep '"tag_name"' | sed -E 's/.*"v?([^"]+)".*/\1/' > "$temp_version" &
            local curl_pid=$!
    ...
    if [[ -n "$latest_version" && "$current_version" != "$latest_version" ]]; then
        ...
        if [[ "$(printf '%s\n' "$current_version" "$latest_version" | sort -V | head -1)" == "$current_version" ]]; then
            export DXAI_UPDATE_AVAILABLE="true"
            printf "  ${GRAY}%s${NC} %-12s ${YELLOW}%s${NC}, running %s\n" "$ICON_WARNING" "dxai" "${latest_version} available" "${current_version}"
        else
            printf "  ${GREEN}✓${NC} %-12s %s\n" "dxai" "Latest version ${current_version}"
        fi
    else
        printf "  ${GREEN}✓${NC} %-12s %s\n" "dxai" "Latest version ${current_version}"
    fi
...
check_login_items() {
    ...
    if [[ -t 0 ]]; then
        ...
        if [[ -t 1 ]]; then
            DXAI_SPINNER_PREFIX="  " start_inline_spinner "Checking login items..."
        fi
...
check_cache_size() {
    ...
    if [[ -t 1 ]]; then
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "Scanning cache..."
    fi
