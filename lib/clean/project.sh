...
readonly PURGE_TARGETS=("${DXAI_PURGE_TARGETS[@]}")
...
readonly DEFAULT_PURGE_SEARCH_PATHS=("${DXAI_PURGE_DEFAULT_SEARCH_PATHS[@]}")

# Config file for custom purge paths.
readonly PURGE_CONFIG_FILE="$HOME/.config/dxai/purge_paths"
...
readonly MONOREPO_INDICATORS=("${DXAI_PURGE_MONOREPO_INDICATORS[@]}")
readonly PROJECT_INDICATORS=("${DXAI_PURGE_PROJECT_INDICATORS[@]}")
...
# dxai Purge Paths - Auto-discovered project directories
...
load_purge_config() {
    ...
    done < <(dxai_purge_read_paths_config "$PURGE_CONFIG_FILE")
...
scan_purge_targets() {
    ...
    local stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/dxai"
...
clean_project_artifacts() {
    ...
    local dry_run_mode="${DXAI_DRY_RUN:-0}"
    for idx in "${selected_indices[@]}"; do
