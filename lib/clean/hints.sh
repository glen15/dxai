...
dxai_hints_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1090
source "$dxai_hints_dir/purge_shared.sh"
...
load_quick_purge_hint_paths() {
    local config_file="$HOME/.config/dxai/purge_paths"
    local -a paths=()

    while IFS= read -r line; do
        [[ -n "$line" ]] && paths+=("$line")
    done < <(dxai_purge_read_paths_config "$config_file")

    if [[ ${#paths[@]} -eq 0 ]]; then
        paths=("${DXAI_PURGE_DEFAULT_SEARCH_PATHS[@]}")
    fi
...
probe_project_artifact_hints() {
    ...
    done < <(dxai_purge_quick_hint_target_names)
