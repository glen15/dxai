...
_clean_mail_downloads() {
    local mail_age_days=${DXAI_MAIL_AGE_DAYS:-}
    ...
    for target_path in "${mail_dirs[@]}"; do
        if [[ -d "$target_path" ]]; then
            ...
            local min_kb="${DXAI_MAIL_DOWNLOADS_MIN_KB:-}"
            if ! [[ "$min_kb" =~ ^[0-9]+$ ]]; then
                min_kb=5120
            fi
...
clean_edge_old_versions() {
    ...
    local -a app_paths
    if [[ -n "${DXAI_EDGE_APP_PATHS:-}" ]]; then
        IFS=':' read -ra app_paths <<< "$DXAI_EDGE_APP_PATHS"
    else
        app_paths=(
...
clean_support_app_data() {
    local support_age_days="${DXAI_SUPPORT_CACHE_AGE_DAYS:-30}"
...
clean_application_support_logs() {
    ...
    local cleaned_count=0
    local found_any=false
    local size_timeout_seconds="${DXAI_APP_SUPPORT_ITEM_SIZE_TIMEOUT_SEC:-0.4}"
    ...
    for app_dir in ~/Library/Application\ Support/*; do
        ...
        for candidate in "${start_candidates[@]}"; do
            if [[ -d "$candidate" ]]; then
                ...
                local quick_count
                # -printf is GNU-only; use portable find | wc -l on macOS
                quick_count=$(command find "$candidate" -mindepth 1 -maxdepth 1 2> /dev/null | wc -l | tr -d ' ')
                ...
                # Batch: calculate size for entire candidate dir at once (1 du call, not N)
                local candidate_size_bytes=0
                local candidate_size_partial=false
                local du_kb
                du_kb=$(du -skP "$candidate" 2> /dev/null | awk '{print $1}') || true
                if [[ "${du_kb:-0}" =~ ^[0-9]+$ ]] && [[ "${du_kb:-0}" -gt 0 ]]; then
                    candidate_size_bytes=$((du_kb * 1024))
                else
                    candidate_size_partial=true
                fi
                local item_found=false
                # Suppress per-item oplog sizing (already tracked at candidate level)
                local _saved_oplog="${MO_NO_OPLOG:-}"
                MO_NO_OPLOG=1
                while IFS= read -r -d '' item; do
                    [[ -e "$item" ]] || continue
                    item_found=true
