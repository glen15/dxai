...
readonly ORPHAN_AGE_THRESHOLD=${ORPHAN_AGE_THRESHOLD:-${DXAI_ORPHAN_AGE_DAYS:-60}}
...
clean_ds_store_tree() {
    ...
    while IFS= read -r -d '' ds_file; do
        ...
        if [[ $file_count -ge $DXAI_MAX_DS_STORE_FILES ]]; then
            break
        fi
...
scan_installed_apps() {
    ...
    local cache_file="$HOME/.cache/dxai/installed_apps_cache"
...
is_bundle_orphaned() {
    ...
    if [[ -n "$bundle_id" ]] && [[ "$bundle_id" =~ ^[a-zA-Z0-9._-]+$ ]] && [[ ${#bundle_id} -ge 5 ]]; then
        ...
        if [[ -z "$ORPHAN_MDFIND_CACHE_FILE" ]]; then
            ORPHAN_MDFIND_CACHE_FILE=$(mktemp "${TMPDIR:-/tmp}/dxai_mdfind_cache.XXXXXX")
        ...
        if [[ ${#file_patterns[@]} -gt 0 ]]; then
            ...
            for item_path in "${file_patterns[@]}"; do
                ...
                for match in "${matches[@]}"; do
                    ...
                    if [[ $iteration_count -gt $DXAI_MAX_ORPHAN_ITERATIONS ]]; then
                        break
                    fi
    ...
    _system_service_app_exists() {
        ...
        if [[ -n "$bundle_id" ]] && [[ "$bundle_id" =~ ^[a-zA-Z0-9._-]+$ ]] && [[ ${#bundle_id} -ge 5 ]]; then
            if [[ -z "$mdfind_cache_file" ]]; then
                mdfind_cache_file=$(mktemp "${TMPDIR:-/tmp}/dxai_mdfind_cache.XXXXXX")
