...
EXPORT_LIST_FILE="$HOME/.config/dxai/clean-list.txt"
CURRENT_SECTION=""
...
WHITELIST_WARNINGS=()
if [[ -f "$HOME/.config/dxai/whitelist" ]]; then
    ...
    done < "$HOME/.config/dxai/whitelist"
else
...
safe_clean() {
    ...
    local permission_start=${DXAI_PERMISSION_DENIED_COUNT:-0}

    local show_scan_feedback=false
    if [[ ${#targets[@]} -gt 20 && -t 1 ]]; then
        show_scan_feedback=true
        stop_section_spinner
        DXAI_SPINNER_PREFIX="  " start_inline_spinner "Scanning ${#targets[@]} items..."
    fi
    ...
    if [[ ${#existing_paths[@]} -gt 10 ]]; then
        ...
        if [[ -t 1 ]]; then DXAI_SPINNER_PREFIX="  " start_inline_spinner "Scanning items..."; fi
    fi
    ...
    if [[ ${#existing_paths[@]} -gt 3 ]]; then
        ...
        if [[ $dir_count -lt 5 && ${#existing_paths[@]} -gt 20 ]]; then
            if [[ -t 1 && "$show_spinner" == "false" ]]; then
                DXAI_SPINNER_PREFIX="  " start_inline_spinner "Scanning items..."
                show_spinner=true
            fi
            ...
            if [[ ${#existing_paths[@]} -gt 0 ]]; then
                for path in "${existing_paths[@]}"; do
                    ...
                    if ((${#pids[@]} >= DXAI_MAX_PARALLEL_JOBS)); then
        ...
        if [[ "$DRY_RUN" != "true" && ${#existing_paths[@]} -gt 0 && -t 1 ]]; then
            DXAI_SPINNER_PREFIX="  " start_inline_spinner "Cleaning..."
    ...
    local permission_end=${DXAI_PERMISSION_DENIED_COUNT:-0}
...
start_cleanup() {
    ...
    export DXAI_CURRENT_COMMAND="clean"
...
# dxai Cleanup Preview - $(date '+%Y-%m-%d %H:%M:%S')
#
# How to protect files:
# 1. Copy any path below to ~/.config/dxai/whitelist
# 2. Run: dxai clean --whitelist
#
# Example:
...
EOF
        ...
        if has_sudo_session; then
            ...
            SYSTEM_CLEAN=false
            echo -e "${GRAY}${ICON_WARNING} System caches need sudo, run ${NC}sudo -v && dxai clean --dry-run${GRAY} for full preview${NC}"
            echo ""
        fi
        return
    fi
...
perform_cleanup() {
    ...
    if [[ $total_size_cleaned -gt 0 ]]; then
        ...
        if [[ "$DRY_RUN" == "true" ]]; then
            ...
            summary_details+=("Use ${GRAY}dxai clean --whitelist${NC} to add protection rules")
        else
