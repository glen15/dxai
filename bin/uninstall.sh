...
readonly DXAI_UNINSTALL_USER_LC_ALL="${LC_ALL:-}"
readonly DXAI_UNINSTALL_USER_LANG="${LANG:-}"
...
readonly DXAI_UNINSTALL_META_CACHE_DIR="$HOME/.cache/dxai"
readonly DXAI_UNINSTALL_META_CACHE_FILE="$DXAI_UNINSTALL_META_CACHE_DIR/uninstall_app_metadata_v1"
readonly DXAI_UNINSTALL_META_CACHE_LOCK="${DXAI_UNINSTALL_META_CACHE_FILE}.lock"
readonly DXAI_UNINSTALL_META_REFRESH_TTL=604800 # 7 days
readonly DXAI_UNINSTALL_SCAN_SPINNER_DELAY_SEC="0.25"
readonly DXAI_UNINSTALL_INLINE_METADATA_LIMIT=8
readonly DXAI_UNINSTALL_INLINE_MDLS_TIMEOUT_SEC="0.08"
...
uninstall_resolve_display_name() {
    ...
    if [[ -f "$app_path/Contents/Info.plist" ]]; then
        ...
        if [[ -n "$DXAI_UNINSTALL_USER_LC_ALL" ]]; then
            md_display_name=$(run_with_timeout 0.04 env LC_ALL="$DXAI_UNINSTALL_USER_LC_ALL" LANG="$DXAI_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        elif [[ -n "$DXAI_UNINSTALL_USER_LANG" ]]; then
            md_display_name=$(run_with_timeout 0.04 env LANG="$DXAI_UNINSTALL_USER_LANG" mdls -name kMDItemDisplayName -raw "$app_path" 2> /dev/null || echo "")
        else
...
uninstall_collect_inline_metadata() {
    ...
    local metadata_date
    metadata_date=$(run_with_timeout "$DXAI_UNINSTALL_INLINE_MDLS_TIMEOUT_SEC" mdls -name kMDItemLastUsedDate -raw "$app_path" 2> /dev/null || echo "")
    ...
    (
        _refresh_debug() {
            if [[ "${MO_DEBUG:-}" == "1" ]]; then
                ...
                echo "[$ts] DEBUG: [metadata-refresh] $*" >> "${HOME}/.config/dxai/dxai_debug_session.log" 2> /dev/null || true
            fi
        }

        ensure_user_dir "$DXAI_UNINSTALL_META_CACHE_DIR"
        ensure_user_file "$DXAI_UNINSTALL_META_CACHE_FILE"
        if [[ ! -r "$DXAI_UNINSTALL_META_CACHE_FILE" ]]; then
            if ! : > "$DXAI_UNINSTALL_META_CACHE_FILE" 2> /dev/null; then
                _refresh_debug "Cannot create cache file, aborting"
                exit 0
            fi
        fi
        if [[ ! -w "$DXAI_UNINSTALL_META_CACHE_FILE" ]]; then
        ...
        if ! uninstall_acquire_metadata_lock "$DXAI_UNINSTALL_META_CACHE_LOCK"; then
        ...
        merged_file=$(mktemp 2> /dev/null) || {
            ...
            uninstall_release_metadata_lock "$DXAI_UNINSTALL_META_CACHE_LOCK"
            rm -f "$updates_file"
            exit 0
        }
        ...
        ' "$updates_file" "$DXAI_UNINSTALL_META_CACHE_FILE" > "$merged_file"

        mv "$merged_file" "$DXAI_UNINSTALL_META_CACHE_FILE" 2> /dev/null || {
            cp "$merged_file" "$DXAI_UNINSTALL_META_CACHE_FILE" 2> /dev/null || true
            rm -f "$merged_file"
        }
...
scan_applications() {
    ...
    ensure_user_dir "$DXAI_UNINSTALL_META_CACHE_DIR"
    ensure_user_file "$DXAI_UNINSTALL_META_CACHE_FILE"
    local cache_source="$DXAI_UNINSTALL_META_CACHE_FILE"
    ...
    (
        ...
        sleep "$DXAI_UNINSTALL_SCAN_SPINNER_DELAY_SEC" 2> /dev/null || sleep 1
    ...
    while IFS='|' read -r app_path display_name bundle_id app_mtime cached_mtime cached_size_kb cached_epoch cached_updated_epoch cached_bundle_id cached_display_name; do
        ...
        if [[ $cache_match == false ]]; then
            ...
            if [[ $cache_age -gt $DXAI_UNINSTALL_META_REFRESH_TTL ]]; then
                needs_refresh=true
            fi
        fi

        if [[ $needs_refresh == true ]]; then
            if [[ $inline_metadata_count -lt $DXAI_UNINSTALL_INLINE_METADATA_LIMIT ]]; then
    ...
    if [[ -s "$cache_snapshot_file" ]]; then
        if uninstall_acquire_metadata_lock "$DXAI_UNINSTALL_META_CACHE_LOCK"; then
            mv "$cache_snapshot_file" "$DXAI_UNINSTALL_META_CACHE_FILE" 2> /dev/null || {
                cp "$cache_snapshot_file" "$DXAI_UNINSTALL_META_CACHE_FILE" 2> /dev/null || true
...
cleanup() {
    local exit_code="${1:-$?}"
    if [[ "${DXAI_ALT_SCREEN_ACTIVE:-}" == "1" ]]; then
        leave_alt_screen
        unset DXAI_ALT_SCREEN_ACTIVE
    fi
...
main() {
    ...
    export DXAI_CURRENT_COMMAND="uninstall"
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            "--whitelist")
                ...
                echo "Whitelist management is currently supported by: dxai clean --whitelist / dxai optimize --whitelist"
                echo "Use 'dxai uninstall --help' for supported options."
                exit 1
                ;;
            -*)
                echo "Unknown uninstall option: $arg"
    ...
    while true; do
        unset DXAI_INLINE_LOADING DXAI_MANAGED_ALT_SCREEN
        ...
        local prompt_timeout="${DXAI_UNINSTALL_RETURN_PROMPT_TIMEOUT_SEC:-3}"
