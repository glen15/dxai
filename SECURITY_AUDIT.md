# dxai Security Reference

Version 1.0.23 | 2026-07-08
...
Put paths in `~/.config/dxai/whitelist`, one per line:
...
Run `dxai clean --dry-run`, `dxai optimize --dry-run`, or the relevant analyze/status command before destructive cleanup. Token usage parsers for Claude, Codex, and Hermes use read-only file/SQLite access and do not modify source logs or session databases.
...
```bash
bats tests/              # all
bats tests/core_safe_functions.bats tests/purge.bats # focused destructive-path safety
```

Release CI builds Go binaries, builds/signs/notarizes the macOS app, publishes GitHub Release assets, updates Sparkle appcast, and attempts Homebrew tap updates. Shell/BATS security suites are local regression gates unless a separate CI workflow is added.
...
Go deps are pinned in `go.mod` and built via GitHub Actions release jobs. Swift app updates use Sparkle 2, Developer ID signing, and Apple notarization for release builds.

## Limitations

System cache cleanup needs sudo, first time you'll get a password prompt. Orphan files wait 60 days before cleanup, use `dxai uninstall` to delete manually if you're in a hurry. No undo, gone is gone, use dry-run first. Only recognizes English names, localized app names might be missed, but falls back to bundle ID.
