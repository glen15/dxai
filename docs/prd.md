...
- [x] Claude/Codex/Gemini 토큰 파싱 (`.jsonl` 로그)
...
#### CLI
- [x] `dxai ai` — 토큰 대시보드 (Claude + Codex + Gemini)
...
- [ ] Codex/Gemini 쿼터 모니터링 (현재 Claude만)
...
- **Gemini**: `~/.gemini/logs/*.jsonl` (토큰)

모든 데이터는 로컬 read-only 파싱. 원본 수정 없음.

## 데이터 안전

- AI 도구 데이터 (`~/.claude/`, `~/.codex/`)는 `dxai clean`으로 절대 삭제 안 함
- 모든 분석은 로컬. 외부 전송 없음
