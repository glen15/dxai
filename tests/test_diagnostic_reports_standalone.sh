...
TMP_EMPTY=$(mktemp -d 2> /dev/null || mktemp -d -t dxai-test 2> /dev/null || echo "")
[[ -z "$TMP_EMPTY" ]] && TMP_EMPTY="/tmp/dxai-test-$$" && mkdir -p "$TMP_EMPTY"
...
TMP_DIAG=$(mktemp -d 2> /dev/null || mktemp -d -t dxai-diag 2> /dev/null || echo "/tmp/dxai-diag-$$")
TMP_APP=$(mktemp -d 2> /dev/null || mktemp -d -t dxai-app 2> /dev/null || echo "/tmp/dxai-app-$$")
...
TMP_DIAG2=$(mktemp -d 2> /dev/null || mktemp -d -t dxai-diag2 2> /dev/null || echo "/tmp/dxai-diag2-$$")
TMP_APP2=$(mktemp -d 2> /dev/null || mktemp -d -t dxai-app2 2> /dev/null || echo "/tmp/dxai-app2-$$")
