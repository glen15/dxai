...
@test "completion bash generates valid bash script" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"_dxai_completions"* ]]
	[[ "$output" == *"complete -F _dxai_completions dxai mo"* ]]
}
...
@test "completion bash can be loaded in bash" {
	run bash -c "eval \"\$(\"$PROJECT_ROOT/bin/completion.sh\" bash)\" && complete -p dxai"
	[ "$status" -eq 0 ]
	[[ "$output" == *"_dxai_completions"* ]]
}
...
@test "completion fish generates valid fish script" {
	...
	[ "$status" -eq 0 ]
	[[ "$output" == *"complete -c dxai"* ]]
	[[ "$output" == *"complete -c mo"* ]]
}

@test "completion fish includes both dxai and mo commands" {
	output="$("$PROJECT_ROOT/bin/completion.sh" fish)"
	dxai_count=$(echo "$output" | grep -c "complete -c dxai")
...
@test "completion auto-install detects zsh" {
	...
	run grep -E "dxai[[:space:]]+completion" "$HOME/.zshrc"
	[ "$status" -eq 0 ]
}
...
@test "completion auto-install detects already installed" {
	...
	echo 'eval "$(dxai completion zsh)"' >"$HOME/.zshrc"
