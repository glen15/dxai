...
@test "remove_dxai deletes manual binaries and caches" {
	mkdir -p "$HOME/.local/bin"
	touch "$HOME/.local/bin/dxai"
	touch "$HOME/.local/bin/mo"
	mkdir -p "$HOME/.config/dxai" "$HOME/.cache/dxai"
...
printf '\n' | "$PROJECT_ROOT/dxai" remove
EOF

	[ "$status" -eq 0 ]
	[ ! -f "$HOME/.local/bin/dxai" ]
	[ ! -f "$HOME/.local/bin/mo" ]
	[ ! -d "$HOME/.config/dxai" ]
	[ ! -d "$HOME/.cache/dxai" ]
}

@test "remove_dxai dry-run keeps manual binaries and caches" {
	mkdir -p "$HOME/.local/bin"
	touch "$HOME/.local/bin/dxai"
	touch "$HOME/.local/bin/mo"
	mkdir -p "$HOME/.config/dxai" "$HOME/.cache/dxai"
...
printf '\n' | "$PROJECT_ROOT/dxai" remove --dry-run
EOF
