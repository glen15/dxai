...
setup_file() {
    ...
    CURRENT_VERSION="$(grep '^VERSION=' "$PROJECT_ROOT/dxai" | head -1 | sed 's/VERSION=\"\\(.*\\)\"/\\1/')"
...
setup() {
    ...
    DXAI_UPDATE_AVAILABLE=false

    export MOCK_BIN_DIR="$BATS_TMPDIR/dxai-mocks-$$"
...
@test "perform_updates handles Homebrew success and dxai update" {
    run bash --noprofile --norc <<'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/manage/update.sh"
...
cat > "$FAKE_DIR/dxai" <<'SCRIPT'
#!/usr/bin/env bash
echo "Already on latest version"
SCRIPT
chmod +x "$FAKE_DIR/dxai"
...
reset_dxai_cache() { echo "DXAI_CACHE_RESET"; }
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Updating dxai"* ]]
    [[ "$output" == *"dxai updated"* ]]
    [[ "$output" == *"DXAI_CACHE_RESET"* ]]
...
set -euo pipefail
DXAI_TEST_BREW_UPDATE_OUTPUT="Updated 0 formulae"
DXAI_TEST_BREW_UPGRADE_OUTPUT="Warning: dxai 1.7.9 already installed"
DXAI_TEST_BREW_LIST_OUTPUT="dxai 1.7.9"
start_inline_spinner() { :; }
stop_inline_spinner() { :; }
brew() {
  case "$1" in
    update) echo "$DXAI_TEST_BREW_UPDATE_OUTPUT";;
    upgrade) echo "$DXAI_TEST_BREW_UPGRADE_OUTPUT";;
    list) if [[ "$2" == "--versions" ]]; then echo "$DXAI_TEST_BREW_LIST_OUTPUT"; fi ;;
  esac
}
...
@test "update_dxai skips download when already latest" {
...
output="Installing dxai...
◎ dxai installed successfully, version 1.23.1"
...
@test "update_dxai with --force reinstalls even when on latest version" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" CURRENT_VERSION="$CURRENT_VERSION" PATH="$HOME/fake-bin:/usr/bin:/bin" TERM="dumb" bash --noprofile --norc << 'EOF'
set -euo pipefail
curl() {
  local out=""
  ...
  if [[ -n "$out" ]]; then
    cat > "$out" << 'INSTALLER'
#!/usr/bin/env bash
echo "dxai installed successfully, version $CURRENT_VERSION"
INSTALLER
    return 0
  fi
...
"$PROJECT_ROOT/dxai" update --force
EOF
...
@test "update_dxai with --nightly uses installer path and passes DXAI_VERSION=main" {
    run env HOME="$HOME" PROJECT_ROOT="$PROJECT_ROOT" CURRENT_VERSION="$CURRENT_VERSION" PATH="$HOME/fake-bin:/usr/bin:/bin" TERM="dumb" bash --noprofile --norc << 'EOF'
set -euo pipefail
curl() {
  local out=""
  ...
  if [[ -n "$out" ]]; then
    cat > "$out" << 'INSTALLER'
#!/usr/bin/env bash
echo "INSTALLER_DXAI_VERSION=${DXAI_VERSION:-}"
echo "dxai installed successfully, version ${DXAI_VERSION:-unknown}"
INSTALLER
    return 0
  fi
...
"$PROJECT_ROOT/dxai" update --nightly
EOF

    [ "$status" -eq 0 ]
    [[ "$output" == *"Downloading nightly installer"* ]]
    [[ "$output" == *"Installing nightly update"* ]]
    [[ "$output" == *"INSTALLER_DXAI_VERSION=main"* ]]
    [[ "$output" == *"Updated to nightly build (main), main"* ]]
}

@test "update_dxai with --nightly is rejected for Homebrew installs" {
    local fake_brew_root="$HOME/fake-homebrew"
    local fake_cellar_bin="$fake_brew_root/Cellar/dxai/9.9.9/bin"
    local fake_path_bin="$HOME/fake-brew-bin"
    mkdir -p "$fake_cellar_bin" "$fake_path_bin"
    touch "$fake_cellar_bin/dxai"
    ln -sf "$fake_cellar_bin/dxai" "$fake_path_bin/dxai"
...
@test "get_homebrew_latest_version prefers brew outdated verbose target version" {
    run bash --noprofile --norc <<'EOF'
set -euo pipefail
DXAI_SKIP_MAIN=1 source "$PROJECT_ROOT/dxai"

brew() {
  if [[ "${1:-}" == "outdated" ]]; then
    echo "glen15/dxai (1.29.0) < 1.30.0"
    return 0
  fi
  if [[ "${1:-}" == "info" ]]; then
    echo "==> glen15/dxai: stable 9.9.9 (bottled)"
    return 0
  fi
  return 0
}
...
brew() {
  ...
  if [[ "${1:-}" == "info" ]]; then
    echo "==> glen15/dxai: stable 1.31.1 (bottled), HEAD"
    return 0
  fi
  return 0
}
