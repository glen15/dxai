...
@test "check.sh script exists and is valid" {
    ...
    run bash -c "grep -q 'dxai Check' '$PROJECT_ROOT/scripts/check.sh'"
    [ "$status" -eq 0 ]
}
...
@test "test.sh script exists and is valid" {
    ...
    run bash -c "grep -q 'dxai Test Runner' '$PROJECT_ROOT/scripts/test.sh'"
    [ "$status" -eq 0 ]
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"Raycast: dxai Clean | Alfred keyword: clean"* ]]
    [[ "$output" == *"Raycast: dxai Status | Alfred keyword: status"* ]]

    local raycast_dir="$HOME/Library/Application Support/Raycast/script-commands"
    [ -d "$raycast_dir" ]

    local clean_script="$raycast_dir/dxai-clean.sh"
    local uninstall_script="$raycast_dir/dxai-uninstall.sh"
    local optimize_script="$raycast_dir/dxai-optimize.sh"
    local analyze_script="$raycast_dir/dxai-analyze.sh"
    local status_script="$raycast_dir/dxai-status.sh"
    ...
    run grep -q '^# @raycast.title dxai Clean$' "$clean_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title dxai Uninstall$' "$uninstall_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title dxai Optimize$' "$optimize_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title dxai Analyze$' "$analyze_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.title dxai Status$' "$status_script"
    [ "$status" -eq 0 ]

    run grep -q '^# @raycast.description Deep system cleanup with dxai$' "$clean_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description Uninstall applications with dxai$' "$uninstall_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description System health checks and optimization$' "$optimize_script"
    [ "$status" -eq 0 ]
    run grep -q '^# @raycast.description Disk space analysis with dxai$' "$analyze_script"
    [ "$status" -eq 0 ]
...
@test "install.sh supports dev branch installs" {
    ...
    [ "$status" -eq 0 ]
    run bash -c "grep -q 'DXAI_VERSION=\"dev\"' '$PROJECT_ROOT/install.sh'"
    [ "$status" -eq 0 ]
}
