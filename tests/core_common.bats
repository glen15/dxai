...
@test "log_info prints message and appends to log file" {
    ...
    local log_file="$HOME/.config/dxai/dxai.log"
    [[ -f "$log_file" ]]
...
@test "rotate_log_once only checks log size once per session" {
    ...
    result=$(HOME="$HOME" DXAI_LOG_ROTATED=1 bash --noprofile --norc -c "source '$PROJECT_ROOT/lib/core/common.sh'; echo \$DXAI_LOG_ROTATED")
    [[ "$result" == "1" ]]
}
...
source "$PROJECT_ROOT/lib/core/common.sh"
DXAI_SPINNER_PREFIX="  " start_inline_spinner "Testing..."
sleep 0.1
stop_inline_spinner
echo "done"
EOF
...
@test "read_key maps j/k/h/l to navigation" {
    run bash -c "export DXAI_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'j' | read_key"
    [ "$output" = "DOWN" ]

    run bash -c "export DXAI_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'k' | read_key"
    [ "$output" = "UP" ]

    run bash -c "export DXAI_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'h' | read_key"
    [ "$output" = "LEFT" ]

    run bash -c "export DXAI_BASE_LOADED=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'l' | read_key"
    [ "$output" = "RIGHT" ]
}
...
@test "read_key respects DXAI_READ_KEY_FORCE_CHAR" {
    run bash -c "export DXAI_BASE_LOADED=1; export DXAI_READ_KEY_FORCE_CHAR=1; source '$PROJECT_ROOT/lib/core/ui.sh'; echo -n 'j' | read_key"
    [ "$output" = "CHAR:j" ]
}
