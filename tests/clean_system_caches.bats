...
setup() {
    ...
    rm -f "$HOME/.cache/dxai/permissions_granted"
}
...
@test "check_tcc_permissions skips in non-interactive mode" {
    ...
    [ "$status" -eq 0 ]
    [[ ! -f "$HOME/.cache/dxai/permissions_granted" ]]
}

@test "check_tcc_permissions skips when permissions already granted" {
    mkdir -p "$HOME/.cache/dxai"
    touch "$HOME/.cache/dxai/permissions_granted"
...
set -euo pipefail
printf '%s\n' "$HOME/CustomProjects" > "$HOME/.config/dxai/purge_paths"
source "$PROJECT_ROOT/lib/core/common.sh"
source "$PROJECT_ROOT/lib/clean/caches.sh"
run_with_timeout() { shift; "$@"; }
safe_clean() { echo "$2|$1"; }
...
EOF
    ...
    rm -rf "$HOME/CustomProjects" "$HOME/.config/dxai" "$fake_bin" "$find_log"
}
...
@test "clean_project_caches skips stalled root scans" {
    ...
    printf '%s\n' "$HOME/SlowProjects" > "$HOME/.config/dxai/purge_paths"
...
source "$PROJECT_ROOT/lib/clean/caches.sh"
MO_TIMEOUT_BIN=""
export DXAI_PROJECT_CACHE_DISCOVERY_TIMEOUT=0.5
export DXAI_PROJECT_CACHE_SCAN_TIMEOUT=0.5
SECONDS=0
clean_project_caches
...
EOF
    ...
    rm -rf "$HOME/.config/dxai" "$HOME/SlowProjects" "$fake_bin"
}
