...
npm() {
    if [[ "$1" == "config" && "$2" == "get" && "$3" == "cache" ]]; then
        echo "/tmp/dxai-custom-npm-cache"
        return 0
    fi
    return 0
}
...
EOF
    ...
    [ "$status" -eq 0 ]
    [[ "$output" == *"npm cache directory|$HOME/.npm/_cacache/*"* ]]
    [[ "$output" == *"npm cache directory (custom path)|/tmp/dxai-custom-npm-cache/_cacache/*"* ]]
    [[ "$output" == *"npm npx cache (custom path)|/tmp/dxai-custom-npm-cache/_npx/*"* ]]
    [[ "$output" == *"npm logs (custom path)|/tmp/dxai-custom-npm-cache/_logs/*"* ]]
    [[ "$output" == *"npm prebuilds (custom path)|/tmp/dxai-custom-npm-cache/_prebuilds/*"* ]]
}
