...
grep -q "^~/" "$HOME/.config/dxai/purge_paths"
EOF
...
mkdir -p "$HOME/.cache/dxai"
echo "0" > "$HOME/.cache/dxai/purge_stats"
...
stats_dir="${XDG_CACHE_HOME:-$HOME/.cache}/dxai"
...
@test "mo purge: accepts --debug flag" {
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 2 '$PROJECT_ROOT/dxai' purge --debug < /dev/null 2>&1 || true
    "
	true
}
...
@test "mo purge: accepts --dry-run flag" {
	...
	run bash -c "
        export HOME='$HOME'
        $timeout_cmd 2 '$PROJECT_ROOT/dxai' purge --dry-run < /dev/null 2>&1 || true
    "
...
@test "mo purge: creates cache directory for stats" {
	...
	bash -c "
        export HOME='$HOME'
        $timeout_cmd 2 '$PROJECT_ROOT/dxai' purge < /dev/null 2>&1 || true
    "

	[ -d "$HOME/.cache/dxai" ] || [ -d "${XDG_CACHE_HOME:-$HOME/.cache}/dxai" ]
}
