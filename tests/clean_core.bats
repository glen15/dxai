...
setup() {
    ...
    mkdir -p "$HOME/Library/Caches" "$HOME/.config/dxai"
    unset TEST_MOCK_BIN
}
...
run_clean_dry_run() {
    ...
    run env HOME="$HOME" DXAI_TEST_MODE=1 PATH="$test_path" \
        "$PROJECT_ROOT/dxai" clean --dry-run
}
...
@test "mo clean --dry-run reports user cache without deleting it" {
    ...
    run env HOME="$HOME" DXAI_TEST_MODE=1 "$PROJECT_ROOT/dxai" clean --dry-run
    [ "$status" -eq 0 ]
...
@test "mo clean honors whitelist entries" {
    ...
    cat > "$HOME/.config/dxai/whitelist" << EOF
