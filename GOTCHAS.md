...
### Hermes에 5시간 세션 사용량을 표시하면 quota로 오해됨
- **발견**: 2026-07-11
- **증상**: 공식 5시간/7일 quota 기준이 없는 Hermes 카드에 `세션(5h)` 토큰이 표시되어 사용 한도처럼 보임
- **근본 원인**: Claude quota 비교용 `recentStats(hours: 5)`가 모든 도구를 공통 집계하면서 Hermes까지 반환함
- **방어**: 최근 5시간 세션 집계는 Claude/Codex만 반환하고, Hermes는 오늘 사용량과 주간 인사이트의 노란 그래프만 표시
- **교훈**: 임의 시간 구간 사용량과 공급자가 정의한 quota 창은 UI에서 같은 지표처럼 표현하면 안 된다
