...
// Mirror mascot body frames (facing left).
var dxaiBodyMirror = [][]string{
	{
		`    /\_/\`,
		`   / o o \___`,
		`  \ =-=   ___\`,
...
// getdxaiFrame renders the animated mascot.
func getdxaiFrame(animFrame int, termWidth int) string {
	dxaiWidth := 15
	maxPos := max(termWidth-dxaiWidth, 0)
...
func renderHeader(m MetricsSnapshot, errMsg string, animFrame int, termWidth int, catHidden bool) (string, string) {
	...
	var mascot string
	if !catHidden {
		mascot = getdxaiFrame(animFrame, termWidth)
	}
