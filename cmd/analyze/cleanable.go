...
// isCleanableDir marks paths safe to delete manually (not handled by dxai clean).
func isCleanableDir(path string) bool {
	if path == "" {
		return false
	}

	// Exclude paths dxai clean already handles.
	if isHandledByDxaiClean(path) {
		return false
	}
...
// isHandledByDxaiClean checks if a path is cleaned by dxai clean.
func isHandledByDxaiClean(path string) bool {
	cleanPaths := []string{
		"/Library/Caches/",
		"/Library/Logs/",
