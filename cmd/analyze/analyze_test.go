...
func TestIsHandledByMoClean(t *testing.T) {
	tests := []struct {
		...
		// Paths dxai clean does NOT handle.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isHandledByDxaiClean(tt.path)
			if got != tt.want {
				t.Errorf("isHandledByDxaiClean(%q) = %v, want %v", tt.path, got, tt.want)
			}
		})
	}
}
...
func TestIsCleanableDir(t *testing.T) {
	tests := []struct {
		...
		// Paths handled by dxai clean (should NOT be cleanable).
