#!/bin/bash
# dxai - Installer for manual installs.
...
resolve_source_dir() {
    if [[ -n "$SOURCE_DIR" && -d "$SOURCE_DIR" && -f "$SOURCE_DIR/dxai" ]]; then
        return 0
    fi
    ...
    if [[ -n "${BASH_SOURCE[0]:-}" && -f "${BASH_SOURCE[0]}" ]]; then
        ...
        if [[ -f "$script_dir/dxai" ]]; then
            SOURCE_DIR="$script_dir"
            return 0
        fi
    fi

    if [[ -n "${CLEAN_SOURCE_DIR:-}" && -d "$CLEAN_SOURCE_DIR" && -f "$CLEAN_SOURCE_DIR/dxai" ]]; then
    ...
    local url="https://github.com/glen15/dxai/archive/refs/heads/main.tar.gz"

    if [[ "$branch" == "dev" ]]; then
        url="https://github.com/glen15/dxai/archive/refs/heads/dev.tar.gz"
    elif [[ "$branch" != "main" ]]; then
        url="https://github.com/glen15/dxai/archive/refs/tags/${branch}.tar.gz"
    fi

    start_line_spinner "Fetching dxai source, ${branch}..."
    if command -v curl > /dev/null 2>&1; then
        if curl -fsSL --connect-timeout 10 --max-time 60 -o "$tmp/dxai.tar.gz" "$url" 2> /dev/null; then
            if tar -xzf "$tmp/dxai.tar.gz" -C "$tmp" 2> /dev/null; then
                stop_line_spinner

                local extracted_dir
                extracted_dir=$(find "$tmp" -mindepth 1 -maxdepth 1 -type d | head -n 1)

                if [[ -n "$extracted_dir" && -f "$extracted_dir/dxai" ]]; then
    ...
    start_line_spinner "Cloning dxai source..."
    if command -v git > /dev/null 2>&1; then
        ...
        if git clone "${git_args[@]}" https://github.com/glen15/dxai.git "$tmp/dxai" > /dev/null 2>&1; then
            stop_line_spinner
            SOURCE_DIR="$tmp/dxai"
            return 0
        fi
...
# Version helpers
get_source_version() {
    local source_dxai="$SOURCE_DIR/dxai"
    if [[ -f "$source_dxai" ]]; then
        sed -n 's/^VERSION="\(.*\)"$/\1/p' "$source_dxai" | head -n1
    fi
}
...
get_source_commit_hash() {
    ...
    curl -fsSL --connect-timeout 3 \
        "https://api.github.com/repos/glen15/dxai/commits/main" 2> /dev/null |
...
get_latest_release_tag() {
    ...
    tag=$(curl -fsSL --connect-timeout 2 --max-time 3 \
        "https://api.github.com/repos/glen15/dxai/releases/latest" 2> /dev/null |
...
get_latest_release_tag_from_git() {
    ...
    git ls-remote --tags --refs https://github.com/glen15/dxai.git 2> /dev/null |
        awk -F/ '{print $NF}' |
        grep -E '^V[0-9]' |
        sort -V |
        tail -n 1
...
get_installed_version() {
    local binary="$INSTALL_DIR/dxai"
    if [[ -x "$binary" ]]; then
        local version
        version=$("$binary" --version 2> /dev/null | awk '/dxai version/ {print $NF; exit}')
        if [[ -n "$version" ]]; then
            echo "$version"
        else
...
resolve_install_channel() {
    ...
    if [[ "${DXAI_EDGE_INSTALL:-}" == "true" ]]; then
        printf 'nightly\n'
        return 0
    fi
...
parse_args() {
    ...
    for i in "${!args[@]}"; do
        ...
        case "$token" in
            latest | main)
                export DXAI_VERSION="main"
                export DXAI_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            dev)
                export DXAI_VERSION="dev"
                export DXAI_EDGE_INSTALL="true"
                version_token="$token"
                unset 'args[$i]'
                ;;
            [0-9]* | V[0-9]* | v[0-9]*)
                export DXAI_VERSION="$token"
                version_token="$token"
                unset 'args[$i]'
                ;;
            *)
...
check_requirements() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list dxai > /dev/null 2>&1; then
        local dxai_path
        dxai_path=$(command -v dxai 2> /dev/null || true)
        local is_homebrew_binary=false

        if [[ -n "$dxai_path" && -L "$dxai_path" ]]; then
            if readlink "$dxai_path" | grep -q "Cellar/dxai"; then
        ...
        if [[ "$is_homebrew_binary" == "true" ]]; then
            ...
            echo -e "${YELLOW}dxai is installed via Homebrew${NC}"
            echo ""
            echo "Choose one:"
            echo -e "  1. Update via Homebrew: ${GREEN}brew upgrade dxai${NC}"
            echo -e "  2. Switch to manual: ${GREEN}brew uninstall --force dxai${NC} then re-run this"
            echo ""
            exit 1
        else
            log_warning "Cleaning up stale Homebrew installation..."
            brew uninstall --force dxai > /dev/null 2>&1 || true
        fi
    fi
...
download_binary() {
    ...
    local url="https://github.com/glen15/dxai/releases/download/V${version}/${binary_name}-darwin-${arch_suffix}"
...
install_files() {
    ...
    if [[ -f "$SOURCE_DIR/dxai" ]]; then
        if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
            ...
            maybe_sudo cp "$SOURCE_DIR/dxai" "$INSTALL_DIR/dxai.new"
            maybe_sudo chmod +x "$INSTALL_DIR/dxai.new"
            maybe_sudo mv -f "$INSTALL_DIR/dxai.new" "$INSTALL_DIR/dxai"

            log_success "Installed dxai to $INSTALL_DIR"
        fi
    else
        log_error "dxai executable not found in ${SOURCE_DIR:-unknown}"
        exit 1
    fi

    if [[ -f "$SOURCE_DIR/dxai" ]]; then
        if [[ "$source_dir_abs" == "$install_dir_abs" ]]; then
            log_success "dxai alias already present"
        else
            maybe_sudo cp "$SOURCE_DIR/dxai" "$INSTALL_DIR/dxai.new"
            maybe_sudo chmod +x "$INSTALL_DIR/dxai.new"
            maybe_sudo mv -f "$INSTALL_DIR/dxai.new" "$INSTALL_DIR/dxai"
            log_success "Installed dxai alias"
        fi
    fi
    ...
    if [[ "$source_dir_abs" != "$install_dir_abs" ]]; then
        maybe_sudo sed -i '' "s|SCRIPT_DIR=.*|SCRIPT_DIR=\"$CONFIG_DIR\"|" "$INSTALL_DIR/dxai"
    fi
...
verify_installation() {
    ...
    if [[ -x "$INSTALL_DIR/dxai" ]] && [[ -f "$CONFIG_DIR/lib/core/common.sh" ]]; then

        if "$INSTALL_DIR/dxai" --help > /dev/null 2>&1; then
            return 0
        else
            log_warning "dxai command installed but may not be working properly"
        fi
    else
...
setup_path() {
    ...
    if [[ "$INSTALL_DIR" != "/usr/local/bin" ]]; then
        ...
        echo ""
        echo "To use dxai from anywhere, add this line to your shell profile:"
...
print_usage_summary() {
    ...
    local message="dxai ${action} successfully"
    ...
    echo "Usage:"
    if [[ ":$PATH:" == *":$INSTALL_DIR:"* ]]; then
        echo "  dxai                           # Interactive menu"
        echo "  dxai clean                     # Deep cleanup"
        echo "  dxai uninstall                 # Remove apps + leftovers"
        echo "  dxai optimize                  # Check and maintain system"
        echo "  dxai analyze                   # Explore disk usage"
        echo "  dxai status                    # Monitor system health"
        echo "  dxai touchid                   # Configure Touch ID for sudo"
        echo "  dxai update                    # Update to latest version"
        echo "  dxai --help                    # Show all commands"
    else
        echo "  $INSTALL_DIR/dxai                           # Interactive menu"
        echo "  $INSTALL_DIR/dxai clean                     # Deep cleanup"
        echo "  $INSTALL_DIR/dxai uninstall                 # Remove apps + leftovers"
        echo "  $INSTALL_DIR/dxai optimize                  # Check and maintain system"
        echo "  $INSTALL_DIR/dxai analyze                   # Explore disk usage"
        echo "  $INSTALL_DIR/dxai status                    # Monitor system health"
        echo "  $INSTALL_DIR/dxai touchid                   # Configure Touch ID for sudo"
        echo "  $INSTALL_DIR/dxai update                    # Update to latest version"
        echo "  $INSTALL_DIR/dxai --help                    # Show all commands"
    fi
    echo ""
}
...
perform_install() {
    ...
    if [[ "${DXAI_EDGE_INSTALL:-}" == "true" ]]; then
        ...
        echo ""
        local branch_name="${DXAI_VERSION:-main}"
...
perform_update() {
    ...
    if command -v brew > /dev/null 2>&1 && brew list dxai > /dev/null 2>&1; then
        ...
        if [[ -f "$SOURCE_DIR/lib/core/common.sh" ]]; then
            ...
            log_error "Cannot update Homebrew-managed dxai without full installation"
            echo ""
            echo "Please update via Homebrew:"
            echo -e "  ${GREEN}brew upgrade dxai${NC}"
            exit 1
        fi
        exit 0
    fi
    ...
    if [[ -z "$installed_version" ]]; then
        log_warning "dxai is not currently installed in $INSTALL_DIR. Running fresh installation."
        perform_install
        return
    fi
    ...
    if [[ -z "$target_version" ]]; then
        log_error "Unable to determine the latest dxai version."
        exit 1
    fi
