...
echo -e "${BLUE}=== dxai Check, ${MODE} ===${NC}\n"

SHELL_FILES=$(find . -type f \( -name "*.sh" -o -name "dxai" \) \
    -not -path "./.git/*" \
...
if [[ "$MODE" == "format" ]]; then
    ...
    if command -v goimports > /dev/null 2>&1; then
        ...
        goimports -w -local github.com/glen15/dxai ./cmd
...
if command -v shellcheck > /dev/null 2>&1; then
    if shellcheck dxai bin/*.sh lib/*/*.sh scripts/*.sh; then
...
if ! bash -n dxai; then
    echo -e "${RED}${ICON_ERROR} Syntax check failed, dxai${NC}\n"
    exit 1
fi
